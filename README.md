...
- **점수 규칙** (`src/lib/attendance.ts` 한 곳에서 계산):
  ...
  - 개인별·학급별 통계와 순위는 **집계 기간별로** 따로 셉니다. 적을수록 높은 순위, 동점은 같은 순위. 기간마다 총점 10위 이내(동점 포함)에 🎁 표시.
- **리셋(새 집계 기간)**: 점수 통계 오른쪽 위 **리셋** → 날짜를 고르면 그날부터 점수·순위를 0에서 새로 셉니다. **기록은 지우지 않고** 기간 경계만 남기므로, 이전 기간 통계는 "집계 기간" 선택에서 계속 볼 수 있고 잘못 리셋해도 `×`로 취소하면 앞 기간과 다시 합쳐집니다. 다음 기간 시작일을 미리 넣어 둬도 됩니다(예: 10/17, 11/17 → `처음~10/16`, `10/17~11/16`, `11/17~`). 화면은 기본으로 오늘이 속한 기간을 보여 줍니다.
  - 경계 날짜는 시트가 아니라 GAS **Script Properties**의 `ATTENDANCE_PERIOD_STARTS`(JSON 배열)에 저장됩니다.
  - 주간 청소 점수는 집계 기간과 상관없이 월~일 주 단위로 계산됩니다.
- **학생 — `/student/attendance`** (대시보드의 "개인별 출결 현황"): 학번+개인코드로 인증 후 내 지각·조퇴·총점 순위, 개인별 통계, 학급별 통계를 봅니다. 학급별 통계는 **본인 순위·점수만 보이고 다른 학생 행은 🔒로 가립니다.** 이번 주 청소 점수가 3점 이상이면 "다음주 청소 확정이에요!" 배너가 뜹니다(교사 화면 개인별 탭·주간 청소 탭에도 표시). GAS가 인증을 다시 확인하고, **다른 학생의 학번·이름은 내려보내지 않고** 요청마다 섞은 익명 키로만 순위 계산에 씁니다.

관련 GAS 액션: `GET_ATTENDANCE_RECORDS`, `SAVE_ATTENDANCE_DAY`, `GET_ATTENDANCE_FOR_STUDENT`, `GET_ATTENDANCE_PERIODS`, `ADD_ATTENDANCE_PERIOD`, `DELETE_ATTENDANCE_PERIOD`.
